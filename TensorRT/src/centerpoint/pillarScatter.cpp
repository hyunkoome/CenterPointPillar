    ...
    int32_t nbInputs, nvinfer1::IExprBuilder& exprBuilder) noexcept
{
    ...
    std::cout << feature_x_size_ << " " << feature_y_size_ << std::endl;
    std::cout << feature_y_size_ << " " << feature_x_size_ << std::endl;
    return output;
}
    ...
    void* /* workspace */, cudaStream_t stream) noexcept
{
    try
    {
        ...
        if (inputType == nvinfer1::DataType::kHALF)
        {
            ...
            status = pillarScatterKernelLaunch(batchSize, maxPillarNum, numFeatures, pillar_features_data,
